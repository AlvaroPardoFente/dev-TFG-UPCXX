...
PROGRAM_PATH="$HOME/dev-TFG-UPCXX/build/remote_completion/"
PROGRAMS=("upcxx_rpc_dist_object" "upcxx_rpc_global_var")
...
for i in "${!NODE_VALUES[@]}"; do
  ...
  for PROGRAM in "${PROGRAMS[@]}"; do
    ...
    if [[ $PROGRAM == mpi* ]]; then
      ...
      sbatch --exclusive -N "$NODES" -n "$NTASKS" --ntasks-per-node=2 -c 1 -t 00:15:00 -p compute0 -o "$OUTPUT_FILE" ~/dev-TFG-UPCXX/"$SBATCH_SCRIPT" --value=sizes.txt "$PROGRAM_PATH""$PROGRAM" --repetitions 200 -m "$MEASUREMENT_MODE" -q --warmup-repetitions 200
    elif [[ $PROGRAM == upcxx* ]]; then
      SBATCH_SCRIPT="new_upcxx_sbatch.sh"
      UPCXX_SHARED_HEAP_SIZE=2G sbatch --exclusive -N "$NODES" -n "$NTASKS" --ntasks-per-node=2 -c 1 -t 00:15:00 -p compute0 -o "$OUTPUT_FILE" ~/dev-TFG-UPCXX/"$SBATCH_SCRIPT" --value=sizes.txt "$PROGRAM_PATH""$PROGRAM" --repetitions 200 -m "$MEASUREMENT_MODE" -q --warmup-repetitions 200
    else
