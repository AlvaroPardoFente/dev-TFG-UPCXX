...
class UpcxxRputIrregular : public UpcxxBenchmarkScheme
{
public:
    ...
    std::vector<std::pair<uint32_t *, size_t>> srcs;
    std::vector<std::pair<upcxx::global_ptr<uint32_t>, size_t>> dsts;
    ...
    UpcxxRputIrregular() : UpcxxBenchmarkScheme(non_contiguous_settings = new NonContiguousSettings()) {}
    ...
    void init(int argc, char *argv[]) override
    {
        ...
        {
            srcs.push_back(std::make_pair(&(value_g->local()[i * block_size]), block_size));
            dsts.push_back(std::make_pair(root_ptr + world_rank * nums_per_rank + i * block_size, block_size));
        }
    };
